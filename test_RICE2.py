...
import torch
import argparse
from utils.visible_images import save_tensor_image
...
decloudnet_weight_path = os.path.join("output_result/RICE2_weights","decloud_net.pth")
...
with torch.no_grad():
    ...
    for data_val in tqdm(test_loader):
        ...
        predict_dir = r"output_image"


        _, _, _, _,img,_,_ = decloud_net(hazy)
        save_tensor_image(img[0], os.path.join(predict_dir, "{:0>4d}_img.jpg".format(iteration)))
        iteration += 1
